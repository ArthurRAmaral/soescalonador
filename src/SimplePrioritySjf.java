...
//        System.out.println(list + "\n\n\n");
		...
		for (int i = 0; i < initSize; i++) {
			...
			if (client.getArrivalTime().compareTo(actual) >= 0) {
				...
				wait = actual.minusHours(client.getArrivalTime().getHour())
						.minusMinutes(client.getArrivalTime().getMinute());
			}

			LocalTime startedAt = actual;
			LocalTime finalizeAt = startedAt.plusHours(client.getEstimatedTime().getHour())
			...
			if (finalizeAt.isBefore(dayEnd)) {
				...
				// dayStart.getMinute()) + (startedAt.getHour() - dayStart.getHour())*60 ));
				responseTimes.add(((wait.getMinute()) + (wait.getHour()) * 60));
				returnTimes.add((((finalizeAt.getMinute() - startedAt.getMinute()) + wait.getMinute())
						+ ((finalizeAt.getHour() - startedAt.getHour()) + wait.getHour()) * 60));
			}
		}
	...
	public int startThread(File database, LocalTime dayStart, LocalTime dayEnd, int qntClients, int loop) {
		...
		ConsumerPrioritySjf clerk2 = new ConsumerPrioritySjf(actual, dayEnd, list, listLock, countItems, (qntClients)/CLERKS, "Isabel");
